  ...
  static {
    ...
    lambda3 = (undefined, function (event) {
      ...
      lambda4 = (undefined, function () {
        let startTime, sourceCode, res, elapsedSeconds, problematicStages, compiledCode, tmp17, tmp18, tmp19, tmp20, tmp21, tmp22, tmp23, tmp24, tmp25, tmp26, tmp27, tmp28, tmp29, tmp30, tmp31, tmp32, tmp33, tmp34, tmp35, tmp36, tmp37, tmp38, tmp39, tmp40, tmp41, tmp42, tmp43, tmp44, tmp45, lambda6;
        tmp17 = Components.clear();
        ...
        tmp29 = Main.showDiagnostics(Components.parser.diagnostics, [
          ...res.lexer.diagnostics,
          ...res.parser.diagnostics
        ], problematicStages);
        Components.parser.trees.innerHTML = "";
        lambda6 = (undefined, function (tree) {
          let tmp46;
          tmp46 = CollapsibleTree.create(tree);
          return runtime.safeCall(Components.parser.trees.appendChild(tmp46))
        });
        tmp30 = Iter.each(res.parser.trees, lambda6);
        Components.elaborator.tree.innerHTML = "";
        tmp31 = CollapsibleTree.create(res.elaborator.tree);
        tmp32 = runtime.safeCall(Components.elaborator.tree.appendChild(tmp31));
        tmp33 = Main.showDiagnostics(Components.elaborator.diagnostics, res.elaborator.diagnostics, problematicStages);
        Components.resolver.tree.innerHTML = "";
        tmp34 = CollapsibleTree.create(res.resolver.tree);
        tmp35 = runtime.safeCall(Components.resolver.tree.appendChild(tmp34));
        tmp36 = Main.showDiagnostics(Components.resolver.diagnostics, res.resolver.diagnostics, problematicStages);
        Components.lowering.tree.innerHTML = "";
        tmp37 = CollapsibleTree.create(res.lowering.tree);
        tmp38 = runtime.safeCall(Components.lowering.tree.appendChild(tmp37));
        tmp39 = Main.showDiagnostics(Components.lowering.diagnostics, res.lowering.diagnostics, problematicStages);
        tmp40 = runtime.safeCall(res.codeGeneration.vars.trim());
        tmp41 = tmp40 + "\n";
        tmp42 = runtime.safeCall(res.codeGeneration.code.trim());
        tmp43 = tmp41 + tmp42;
        compiledCode = tmp43;
        tmp44 = Main.renderGeneratedCode(compiledCode);
        tmp45 = Main.showDiagnostics(Components.codegen.diagnostics, res.codeGeneration.diagnostics, problematicStages);
      ...
      lambda5 = (undefined, function (error) {
        ...
        return Components.errorDisplay.show(error)
      });
